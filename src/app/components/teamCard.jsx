...
export default function TeamCard({ name, role, picture, description }) {
    return (
        <motion.div initial={{ opacity: 0, y: -30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ amount: 0.6, once: true }} whileHover={{ y: -15 }} className="relative rounded-lg overflow-clip min-h-[590px] min-w-[336px] max-h-[590px] max-w-[336px]">
            <Image src={picture} width={2552} height={3828} alt={name} className="select-none" />
            <div className="bg-[#181818] h-full w-[150%] rotate-[8deg] absolute bottom-[-290px] right-[-120px]"></div>
            <div className="flex flex-col absolute top-39/72 px-6">
