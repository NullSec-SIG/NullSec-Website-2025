...
export default function WorkshopCard({ name, image, date, time, status, description, location }) {
    ...
    return (
        <motion.div initial={{ opacity: 0, y: -30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ amount: 0.6, once: true }} onHoverStart={() => setHover(true)} onHoverEnd={() => setHover(false)} className="w-[384px] h-[480px] rounded-2xl overflow-clip cursor-pointer relative">
